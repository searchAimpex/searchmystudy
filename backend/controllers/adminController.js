...
const extraUser = asyncHandler(async (req, res) => {
  const { name, email, password, role, createdBy, WhatappNumber } = req.body;
  console.log(req.body,"????????????????????????????");
  // File uploaded via multer.single('ProfilePhoto')
  const profilePhotoPath = req.file ? `upload/${req.file.filename}` : '';
  ...
  // console.log(userByWhatsAppNumber,"????????????????????????????");
...
const createProfile = async (req, res) => {
  try {
    ...
    // if (existing) {import asyncHandler from 'express-async-handler';
    //   import Banner from "../models/bannerModel.js";
    //   import Service from '../models/servicesModel.js';
    //   import Testimonial from '../models/testimonialsModel.js';
    //   import Counsellor from '../models/counsellorModel.js';
