...
import { useTestimonialFetchAllMutation, useServiceDeleteMutation, useTestimonialDeleteMutation, useCreateCounsellorMutation, useAllCounsellorMutation, useCounsellorDeleteMutation, useAllVideoMutation, useVideoDeleteMutation } from '../../slices/adminApiSlice';
...
import CreateCounsellorPop from './PopUps/CreateCounsellorPop';
...
function EnhancedTableToolbar({ numSelected, selectedRow, onViewBanner, onDelete }) {
    ...
    return (
        ...
        >
            ...
            <CreateVideoPop open={open} handleClose={handleClose} />
...
function VideoTable() {
    ...
    const services = video || []; // Assuming `testimonial` state holds the array of testimonials
    const [AllVideo, { isSuccess }] = useAllVideoMutation();
    const [CounsellorDelete, DeleteState] = useVideoDeleteMutation();
    ...
    const handleDelete = async (id) => {
        try {
            const res = await CounsellorDelete(id).unwrap();
            dispatch(deleteCounsellor(res));
        } catch (error) {
