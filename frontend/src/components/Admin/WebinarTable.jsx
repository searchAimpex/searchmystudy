...
import { useTestimonialFetchAllMutation, useServiceDeleteMutation, useTestimonialDeleteMutation, useAllWebinarMutation, useDeleteWebinarMutation } from '../../slices/adminApiSlice';
...
function EnhancedTableToolbar({ numSelected, selectedRow, onViewBanner, onDelete }) {
    ...
    return (
        ...
        >
            ...
            <CreateWebinarPop open={open} handleClose={handleClose} />
...
function WebinarTable() {
    ...
    const services = webinar || []; // Assuming `testimonial` state holds the array of testimonials
    const [AllWebinar, { isSuccess }] = useAllWebinarMutation();
    const [DeleteWebinar, DeleteState] = useDeleteWebinarMutation();
    ...
    const handleDelete = async (id) => {
        try {
            const res = await DeleteWebinar(id).unwrap();
            dispatch(DeleteOneWebinar(res));
        } catch (error) {
