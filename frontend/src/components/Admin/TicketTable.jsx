...
import { useDeleteProvinceMutation, useFetchAllTicketMutation, useFetchProvinceMutation, useRemoveOneTicketMutation, useUpdateOneTicketMutation } from '../../slices/adminApiSlice.js';
...
const headCells = [
    ...
    { id: 'createBy', numeric: false, disablePadding: false, label: 'Created By' },
...
function EnhancedTableToolbar({ numSelected, selectedRow, onViewBanner, onDelete }) {
    ...
    return (
        ...
        >
            ...
            <UpdateTicketStatus open = {openStatus}  handleClose = {handleStatusClose} ticketId =  {selectedRow?._id}/>
            <ViewTicketPop open={openViewTicket} handleClose={handleCloseViewTicket} ticketData={selectedRow || ''} />
        </Box>
    );
}
...
function TicketTable() {
    ...
    const [FetchAllTicket , { isSuccess }] = useFetchAllTicketMutation()
    const [RemoveOneTicket,deleteStatus] = useRemoveOneTicketMutation()
    const [UpdateOneTicket,updateStatusNotify] = useUpdateOneTicketMutation()
    ...
    useEffect(() => {
        ...
        if(updateStatusNotify?.isSuccess){
            toast.success('Data updated successfully');
        }
    
    }, [isSuccess,deleteStatus?.isSuccess,updateStatusNotify?.isSuccess]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await FetchAllTicket().unwrap();
                dispatch(AllTicket(result));
            } catch (error) {
                console.error('Failed to fetch services:', error);
            }
        };
        fetchData();
    }, [FetchAllTicket, dispatch]);
    ...
    const handleDelete = async (id) => {
        try {
            const res = await RemoveOneTicket(id).unwrap();
            dispatch(DeletetTicket(res))
        } catch (error) {
