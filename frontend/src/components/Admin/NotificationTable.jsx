...
import { useTestimonialFetchAllMutation, useServiceDeleteMutation, useTestimonialDeleteMutation, useAllWebinarMutation, useDeleteWebinarMutation, useFetchMediaMutation, useDeleteMediaMutation, useFetchNotifcationMutation, useFetchAllNotifcationMutation } from '../../slices/adminApiSlice';
...
import CreateNotificationPop from './PopUps/CreateNotificationPop';
import { AddNotifcation } from '../../slices/notificationSlice';

const headCells = [
    { id: '_id', numeric: false, disablePadding: true, label: 'ID' },
    { id: 'role', numeric: false, disablePadding: false, label: 'Title' },
    { id: 'message', numeric: false, disablePadding: false, label: 'Description' },
...
function EnhancedTableToolbar({ numSelected, selectedRow, onViewBanner, onDelete }) {
    ...
    return (
        ...
        >
            ...
            <CreateNotificationPop open={open} handleClose={handleClose} />
        </Box>
    );
}
...
function NotificationTable() {
    ...
    const services = notification || []; // Assuming `testimonial` state holds the array of testimonials
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await fetchAllNotifcation().unwrap();
                dispatch(AddNotifcation(result));
            } catch (error) {
                console.error('Failed to fetch testimonials:', error);
            }
        };
        fetchData();
    }, [fetchAllNotifcation, dispatch]);
