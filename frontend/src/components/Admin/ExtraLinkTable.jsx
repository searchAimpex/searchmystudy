...
import { useGetAllBannerMutation,useDeleteBannerMutation, useGetAllPromotionalMutation, useDeletePromotionalMutation, useFetchNavMutation, useDeleteMyNavMutation } from '../../slices/adminApiSlice';
      ...
      {numSelected > 0 ? (
        ...
        <Tooltip title="Create Banner">
          <IconButton size="sm" variant="outlined" color="neutral">
            ...
            <CreateExtraPop open={open} handleClose={handleClose} />
          </IconButton>
        </Tooltip>
      )}
    ...
    const deleteSelectedBanners = async () => {
      try {
        const res = await Promise.all(selected.map(async (id) => {
          await DeleteMyNav(id);
          return id; 
        }));
        console.log("Delete res",res);
        dispatch(DeleteOneNavs(res))
        setSelected([]);
      } catch (error) {
