...
import { useDeleteMyUploadMutation, useDeleteProvinceMutation, useFetchAllTicketMutation, useFetchMyCommissionMutation, useFetchMyUploadMutation, useFetchProvinceMutation, useRemoveOneTicketMutation, useUpdateOneTicketMutation } from '../../slices/adminApiSlice.js';
import { DeleteOneProvince,  FetchProvinces } from '../../slices/provinceSlice.js';
import { AllTicket, DeletetTicket } from '../../slices/ticketSlice.js';
import UpdateTicketStatus from './PopUps/UpdateTicketStatus.jsx';
import ViewTicketPop from './PopUps/ViewTicketPop.jsx';
import CreateUploadPop from './PopUps/CreateUploadPop.jsx';
import { DeleteUploadState, FetchUpload } from '../../slices/uploadSlice.js';
import { Button } from '@mui/material';
import CreateCommissionPop from './PopUps/CreateCommissionPop.jsx';
import { FetchCommission } from '../../slices/commissionSlice.js';
...
const headCells = [
    ...
    { id: 'updatedAt', numeric: false, disablePadding: false, label: 'Update At' },
];
            ...
            {numSelected > 0 ? (
                <div className='flex flex-row justify-between w-[150px]'>
                    ...
                    <Tooltip title="Edit Status">
                        <IconButton size="sm" color="danger" variant='solid' >
                            <EditIcon onClick = {()=>handleStatusOpen()}  />
                        </IconButton>
                    </Tooltip>
                    <Tooltip title="View Hero Image">
                        <IconButton  size="sm" color="danger" variant="solid">
                            <RemoveRedEye onClick={()=>handleOpenViewTicket()}/>
                ...
                <Tooltip title="Add File">
                    <IconButton size="sm" variant="outlined" color="danger" onClick={handleClickOpen}>
                        <AddIcon />
                        <CreateCommissionPop open={open} handleClose={handleClose} />
                    </IconButton>
                </Tooltip>
            )}
...
function CommissionTable() {
    ...
    const services  = commission;
    const [FetchMyCommission , { isSuccess }] = useFetchMyCommissionMutation()
    const [DeleteMyUpload,deleteStatus] = useDeleteMyUploadMutation()
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const result = await FetchMyCommission().unwrap();
                dispatch(FetchCommission(result));
            } catch (error) {
                console.error('Failed to fetch services:', error);
            }
        };
        fetchData();
    }, [FetchMyCommission, dispatch]);
    ...
    const handleDelete = async (id) => {
        try {
            const res = await DeleteMyUpload(id).unwrap();
            dispatch(DeleteUploadState(res))
        } catch (error) {
                        ...
                        ?.map((row, index) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td id={labelId}>{row?._id?.slice(0,5)}</td>
                                    <td>{row?.target}</td>
                                    <td>{row?.SecondCountry?.name}</td>
                                    <td>
                                        {row?.fileURL ? (
                                            <button
